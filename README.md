...
- 일반 스탠바이: 왼쪽(또는 위쪽)에 시계, 오른쪽(또는 아래쪽)에 정보 위젯을 표시
- 일반 스탠바이에서 좌우 스와이프 시계 스타일(Digital Bold/Minimal/Flip/Analog), 상하 스와이프 오른쪽 위젯(위젯/달력/알림) 변경
- 듀얼 화면 세션에서만 내부 화면 무드등을 표시하며, 상하 스와이프로 무드등 프리셋 변경
...
- 일반 스탠바이 좌우 스와이프: 시계 스타일 변경
- 일반 스탠바이 위아래 스와이프: 오른쪽 위젯(위젯/달력/알림) 변경
- 듀얼 화면 세션 위아래 스와이프: 내부 화면 무드등 프리셋 변경
